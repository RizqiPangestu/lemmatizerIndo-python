...
class Lemmatizer():
    ...
    def lookup(self, word):
        ...
        if len(word) <= 6:
            ...
            reg = "^(?<first>aneka|({}{}))(?<second>{}{}(?U)({})*)$".format(syllabel,
                                                                            syllabel, syllabel, syllabel, syllabel)  # notsure if true
                ...
                if prefix_type:
                    ...
                    if prefix == 'be':
                        ...
                        if self.removed['derivational_prefix'] != '':
                            ...
                            array_key = list(self.complex_prefix_tracker.keys())[
    ...
    def recode(self, word):
        ...
        for prefix, changes in prefixes.items():
            ...
            if prefix_added != '':
                result = pcre.sub('^{}'.format(prefix_added),
                                  prefix_removed, result)
            else:
        ...
        for step in steps:
            if step == 3:
                ...
                for prefix, changes in prefixes.items():
                    ...
                    if prefix_added != '':
                        temp = pcre.sub('^{}'.format(
                            prefix_added), prefix_removed, temp)
